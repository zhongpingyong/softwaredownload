...
function baseResponseMsg(code: HttpStatusCode, errMsg: string): IBaseResponse<undefined> {
  return {
    code,
    errMsg,
    data: undefined
...
baseService.interceptors.response.use(
  (response) => {
    const res = response.data as IBaseResponse<any>
...
export function requestData<T>({
  ...
  params = {},
  data = {}
}: RequestData): Promise<IBaseResponse<T>> {
